...
1. **Naive fix (insufficient on its own)**: raise `CTX_SIZE` in `qwen38.env` and `./serve-qwen38.sh restart`. Stay well clear of ~80K (llama.cpp#27623 — see the known-bugs table). Recurred three times as this alone (32768 → 65536 → 73728).
...
5. **The actual fix: cap how much a single turn can add, don't just hope compaction catches it in time.** `qwen-code`'s per-tool-call `tools.truncateToolOutputThreshold` (stock default 25,000 characters) still lets a *batch* of several fanned-out tool calls (e.g. reading many files at once) add up to far more than that in one turn. `wire-qwen-code` now also sets `tools.truncateToolOutputThreshold=8000`, `tools.truncateToolOutputLines=300`, and `model.sessionTokenLimit=50000` (a deterministic backstop — this one drops the next send outright once the recorded prompt count is already over the limit, rather than depending on token-count estimation at all). These are global `settings.json` fields, not provider-specific, and **require a fresh `qwen` session to take effect** (not just Ctrl+Y retry).
...
If it recurs even with all of the above, that's a strong signal the session's genuine, non-tool-output conversation content (the back-and-forth text itself) has grown past a comfortable working set — starting a fresh `qwen` session is the sustainable move, not raising `CTX_SIZE` further toward the ~80K decode-collapse cliff.
